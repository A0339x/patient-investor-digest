  ...
  },
  {
    ...
    "stories": [
      ...
      },
      {
        ...
        "body": "Exponent, a Solana-based yield exchange (a protocol that lets users trade or lock in future yield at a fixed rate rather than accepting variable returns), raised $5 million in seed funding led by Multicoin Capital. Every new yield product pulls from the same pool of yield-seeking dollars that currently sits in LP positions. The primary variable here is TVL. When a competing strategy gets attractive enough that capital rotates out of LP pools, TVL in those pools drops -- and the LPs who stay capture a larger share of the same fee pie. The non-obvious second variable is Volume. APR is essentially Volume times fee tier divided by TVL. So a TVL drop only translates to higher APR for the LPs who stayed if Volume holds steady. When a new yield product makes headlines for raising capital, keep in mind that two things are now in motion in opposite directions: TVL on the affected LP pools may soften (which would lift APR for whoever stays), and Volume may or may not hold (the part that decides whether staying was actually rewarded). The headline reads bullish for LPs already in deep pools, but it's only bullish if both halves of the equation cooperate.",
